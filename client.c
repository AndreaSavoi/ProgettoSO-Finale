...
//
            ...
            scanf_retry_seats:
                ...
                while(singleSeatBuff) {				//Checking if seats are valid with all controls
                    ...
                    if(singleSeatBuff != NULL) {n_check++;}
                }

                if(n_check != n_seats) {

                  printf("[-]Your reservation includes %d seats, but you are trying to reserve a different number of seats (%d seats)\nTry again please...\n", n_seats, n_check);
                  goto scanf_retry_seats;
                }
...
//
//CASE 4: SHOW ACTIVE RESERVATIONS
//
            ...
            case 4:
                ...
                puts("Your current active reservations will now be displayed. Notice that, when deleting using a code, all seats with the said code will be deleted\n");
...
int ParseCmdLine(int argc, char *argv[], char **szAddress) {
    ...
    {
        printf("Syntax:\n\n");
        printf("    client -a (Remote address) [-h].\n\n");
        exit(EXIT_SUCCESS);
    }
    return 0;
}
